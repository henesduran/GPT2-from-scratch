...
- **Data:** Turkish Wikipedia (`wikimedia/wikipedia`, config `20231101.tr`), streamed and tokenized with the existing `scripts/prepare_data.py` (no modifications needed — it's dataset-agnostic).
...
- **Hardware:** single RTX 4070 Laptop GPU, 8GB VRAM. `batch_size=4, block_size=1024` throughout — no OOM at any point in either sweep. This repo has no gradient checkpointing, so LoRA's memory savings here are in optimizer state only, not activations, which is why batch size had to stay conservative.
...
1. **Turkish adaptation quality improves monotonically with rank, at both data scales.** Validation loss on held-out Turkish text drops steadily from r=1 to r=32 (3.87→3.23 at the MVP scale, 3.59→3.01 at full scale) — more capacity keeps helping across the whole swept range, no sign of saturation yet even at r=32.
